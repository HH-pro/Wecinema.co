...
import { deleteRequest, getRequest, putRequest, changeUserType, getUserProfile, updateUserProfile } from "../api";
...
const GenrePage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const result = await getUserProfile(id, setLoading);
                setUser(result);
                setMarketplaceMode(result.userType || 'buyer');
                setFormData({ 
    ...
    useEffect(() => {
        ...
        const fetchVideos = async () => {
            try {
                const result: any = await getRequest(`video/authors/${id}/videos`, setLoading);
                if (result) {
                    setVideos(result);
                }
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const result = await updateUserProfile(id, formData, setLoading);
            setUser(result.user);
            setEditMode(false);
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex space-x-3 mt-4 sm:mt-0">
                                ...
                                <button 
                                    ...
                                    disabled={changingMode || !isCurrentUser}
                                    ...
                                    } ${changingMode || !isCurrentUser ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                                    {changingMode ? (
