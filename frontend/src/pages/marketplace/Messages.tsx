// src/pages/Messages.tsx - UPDATED WITH YELLOW-500 BUTTONS
...
import { FiMessageSquare, FiRefreshCw, FiShoppingBag, FiUser, FiChevronLeft } from 'react-icons/fi';
...
const Messages: React.FC = () => {
  ...
  // ✅ UPDATED: Yellow-500 buttons in empty state
  const renderEmptyState = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
      <div className="w-32 h-32 mx-auto mb-6 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-full flex items-center justify-center">
        <FiMessageSquare className="w-16 h-16 text-yellow-500" />
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        {/* ✅ UPDATED: Yellow-500 button */}
        <button
          onClick={() => navigate('/marketplace')}
          className="px-6 py-3 bg-yellow-500 text-white font-semibold rounded-lg hover:bg-yellow-600 transition-colors shadow-sm hover:shadow flex items-center justify-center"
        >
          <FiShoppingBag className="w-5 h-5 mr-2" />
          Browse Marketplace
        </button>
        {/* ✅ UPDATED: Yellow-500 outline button */}
        <button
          onClick={handleRefreshChats}
          className="px-6 py-3 border-2 border-yellow-500 text-yellow-600 font-semibold rounded-lg hover:bg-yellow-50 transition-colors flex items-center justify-center"
        >
    ...
    <MarketplaceLayout>
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-6 sm:py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* ✅ UPDATED: Yellow-500 refresh button */}
                <button
                  onClick={handleRefreshChats}
                  className="inline-flex items-center px-4 py-2 bg-yellow-500 text-white font-medium rounded-lg hover:bg-yellow-600 transition-colors shadow-sm hover:shadow"
                >
          ...
          {/* Main Content */}
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 h-[calc(100vh-180px)] flex flex-col lg:flex-row overflow-hidden">
            {/* Chat List */}
              ...
              <div className="flex-1 overflow-y-auto">
                {chatsLoading ? (
                  ...
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <button
                      onClick={() => navigate('/marketplace')}
                      className="mt-3 px-4 py-2 bg-yellow-500 text-white text-sm font-medium rounded-lg hover:bg-yellow-600 transition-colors"
                    >
                      Browse Marketplace
                    </button>
                  </div>
              ...
              {selectedChat && selectedChat.otherUser ? (
                <>
                  ...
                  <div className="border-b border-gray-200 p-4 bg-gradient-to-r from-gray-50 to-white">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center space-x-2">
                        {selectedChat.order?._id && (
                          // ✅ UPDATED: Yellow-500 view order button
                          <button
                            onClick={() => navigate(`/orders/${selectedChat.order!._id}`)}
                            className="px-4 py-2 text-sm font-medium text-yellow-700 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors border border-yellow-300"
                          >
                            View Order
                          </button>
                        )}
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8">
                  {chats.length === 0 ? (
                    ...
                    <>
                      <div className="w-48 h-48 mx-auto mb-6 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-full flex items-center justify-center">
                        <FiMessageSquare className="w-24 h-24 text-yellow-500" />
                      </div>
