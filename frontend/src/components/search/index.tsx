...
const Search: React.FC<ThemeProps> = ({
    title,
    isFirst,
    search,
    type,
        ...
        const fetchVideos = async () => {
            ...
            try {
                const result: any = search
                    ? await getRequest(`video/search/${search}`, setLoading)
    ...
    const filterVideos = (search?: string) => {
        if (!search) return videos;
    
        const lowerSearch = search.toLowerCase();
    
        return videos.filter((v: any) =>
            (v.title && v.title.toLowerCase().includes(lowerSearch)) ||
            (v.description && v.description.toLowerCase().includes(lowerSearch)) ||
            (Array.isArray(v.genre) && v.genre.some((g: string) => g.toLowerCase().includes(lowerSearch))) ||
            (Array.isArray(v.theme) && v.theme.some((t: string) => t.toLowerCase().includes(lowerSearch)))
        );
    };
    ...
    const filtered = filterVideos(search);

    if (loading) {
        return (
            <div className={`${isFirst ? "mt-5" : ""} z-1 relative p-2 flex flex-wrap border-b overflow-hidden border-blue-200 sm:mx-4 pb-4`}>
    ...
    if (filtered.length === 0 && !loading && type === "profile") {
        return (
            <div className={`${isFirst ? "mt-5" : ""} z-1 relative p-2 flex text-center flex-wrap border-b overflow-hidden border-blue-200 sm:mx-4 pb-4`}>
            ...
            <div className="flex flex-wrap w-full">
                {filtered.map((video: any, index: number) => (
                    <div
                        ...
                        className="cursor-pointer gallery relative flex-wrap  border-gray-200  w-full   p-2 "
                    >
                        <div onClick={() => handleVideolick(video)} className="thumbnail relative overflow-hidden">
                            <VideoThumbnail
                                videoUrl={video.file}
                                className="border-gray-200 rounded-xl w-full"
                            />
                        </div>
                        <div className="footer flex-1 block" onClick={() => nav("/user/" + video?.author?._id)}>
