...
const VideoPlayerModal: React.FC<VideoPlayerModalProps> = ({
  ...
  useEffect(() => {
    if (show && videoUrl) {
      console.log('🎬 Modal Mounted/Updated:', {
        videoUrl,
        showThumbnail,
        hasError,
        isLoading
  ...
  // Fix: Thumbnail ko hide karne ka logic
  useEffect(() => {
    if (isPlaying) {
  ...
  const generateThumbnailUrl = (url: string): string => {
    ...
    if (videoThumbnail && videoThumbnail !== '') {
      console.log('🎬 Using provided thumbnail:', videoThumbnail);
    ...
    // Return a solid color placeholder with play icon
  ...
  const handleThumbnailClick = () => {
    ...
    if (videoRef.current) {
      videoRef.current.play()
        ...
        .catch(error => {
          console.error('🎬 Error playing video from thumbnail:', error);
          setHasError(true);
        });
    }
  };

  // FIX: Modal show nahi hai ya URL nahi hai to return null
        ...
        <div className="relative w-full h-[70vh] bg-black">
          {isVideoUrl(videoUrl) && !hasError ? (
            <>
              ...
              {/* Video Element - ALWAYS RENDER BUT MAY BE HIDDEN */}
              <video
                ref={videoRef}
                src={videoUrl}
                controls={false}
                autoPlay={false}
                muted={isMuted}
                className={`w-full h-full object-contain ${showThumbnail ? 'opacity-0' : 'opacity-100'}`}
                onPlay={() => {
              ...
              {/* Loading Overlay - ONLY WHEN LOADING AND NOT SHOWING THUMBNAIL */}
              ...
              {/* Custom Video Controls - ONLY SHOW WHEN VIDEO IS PLAYING */}
              {!showThumbnail && (
