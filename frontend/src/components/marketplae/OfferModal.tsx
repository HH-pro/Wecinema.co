...
const OfferModal: React.FC<OfferModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto">
      ...
      <div className="relative mt-4 sm:mt-8 lg:mt-12 bg-white w-full max-w-md sm:max-w-lg rounded-xl shadow-2xl flex flex-col max-h-[calc(100vh-4rem)] sm:max-h-[calc(100vh-6rem)] overflow-hidden transition-all duration-200 ease-out">
        
        {/* Header - Always visible at top */}
        <div className="sticky top-0 z-20 bg-gradient-to-r from-yellow-50 to-white border-b border-gray-200 p-3 sm:p-4 flex justify-between items-center">
          <div>
        ...
        {/* Scrollable Body with padding for content */}
        <div className="flex-1 overflow-y-auto">
          <div className="px-3 sm:px-5 py-3 space-y-4">
            ...
            {/* Offer Form */}
            <form onSubmit={onSubmit} className="space-y-3 pb-20">
              {/* Amount */}
              <div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 sm:p-4 shadow-lg">
          <div className="flex flex-col sm:flex-row gap-2">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-100 transition-all text-xs sm:text-sm font-medium"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 py-2.5 rounded-md bg-yellow-600 hover:bg-yellow-700 disabled:bg-gray-400 text-white transition-all text-xs sm:text-sm font-medium flex items-center justify-center gap-2"
            >
